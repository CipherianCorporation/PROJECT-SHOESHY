...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query("SELECT u FROM User u WHERE u.reset_pwd_token = ?1 AND u.isDeleted = false")
    public Optional<User> findByResetPasswordToken(String token);

    @Query("SELECT o FROM User o WHERE o.verify_code=?1 AND o.isDeleted = false")
    ...
    @Modifying
    @Query("UPDATE User u SET u.provider = ?2 WHERE u.email = ?1 AND u.isDeleted = false")
